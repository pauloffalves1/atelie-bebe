...
- `ngsw-config.json`: grupo `app` (`installMode: prefetch`) para o shell (HTML/CSS/JS/favicon); grupo `assets` (`installMode: lazy`) para imagens/ícones; um `dataGroup` para `/api/products/**` com estratégia `freshness` (rede primeiro, timeout de 3s, só cai pro cache se a rede falhar/demorar) — deliberadamente conservador para não arriscar mostrar preço desatualizado, só ajuda em conexão instável/offline.
