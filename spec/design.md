...
- **Antes/depois nos campos alterados** (acrescentado depois do feedback de que só "o quê" sem os valores era pouco útil): `AuditDiff` (`Api/Common/`) — `Field(nome, antigo, novo)` retorna `null` se os dois valores forem iguais (nada a reportar) ou uma string `"nome: antigo → novo"` caso contrário; `Join(...)` combina vários `Field(...)`, descartando os `null`, com um fallback `"sem alterações"` se nada mudou. É comparação explícita campo a campo (não reflection sobre o DTO inteiro) de propósito — um diff genérico por reflection acabaria comparando também campos computados que não fazem parte do formulário de edição (ex. `IsOnPromotion`, `EffectivePrice`, `ImageUrls` em `ProductDto`), gerando ruído por coincidência de timing em vez de mudanças reais.
- Cada endpoint que ganhou o diff busca o registro **antes** da operação de escrita (`GetByIdAsync`) só para ter esse "antes" — uma consulta extra que se paga pela rastreabilidade completa. `ProductUpdated` compara Nome/Descrição/Preço/Categoria/Destaque; `CustomerUpdated` compara Nome/E-mail/CPF/Telefone; `ProductActiveChanged`/`CouponActiveChanged` mostram "ativo ↔ inativo"; `OrderStatusChanged` mostra o status antigo e novo; `OrderTrackingCodeSet` mostra o código antigo (ou "(vazio)") e o novo. `ICouponService` ganhou `GetByIdAsync` (só tinha `ListAsync`/`SetActiveAsync`) para dar esse "antes" ao endpoint de cupom.
...
- `client/e2e/store-purchase.spec.ts` (Playwright, projeto `chromium` apenas): abre a página de um produto real do catálogo de seed, preenche o texto de bordado e escolhe uma cor de linha (os dois campos obrigatórios), adiciona ao carrinho, vai para `/carrinho` e confirma visualmente que o item aparece, segue para "Finalizar compra". Como `/checkout` exige cliente autenticado (`customerGuard`) — o teste inicialmente assumia checkout de convidado, corrigido depois de rodar de verdade e ver o redirect para `/entrar?returnUrl=%2Fcheckout` — o teste então cria uma conta nova (`/cadastro`, chegando lá pelo link "Cadastre-se" a partir da tela de login) com um CPF gerado com dígito verificador válido e um e-mail único por execução (timestamp), volta automaticamente para `/checkout` via `returnUrl`, preenche o formulário e confirma o pedido, terminando na verificação do heading "Pedido recebido!" em `/pedido/:id`.
- `playwright.config.ts`: `webServer` aponta para `npm start` com `reuseExistingServer: true` — sobe o `ng serve` sozinho se não estiver rodando, ou reaproveita um já ativo (útil em desenvolvimento, onde o `ng serve` já costuma estar de pé). O backend (.NET) não é gerenciado pelo Playwright — precisa já estar rodando em `http://localhost:5120`, documentado no `README.md`.
- **Problema de ambiente encontrado e contornado**: `npx playwright test` falhava consistentemente com `Playwright Test did not expect test() to be called here` / `Error: No tests found`, mesmo com um teste mínimo em JavaScript puro (isolando qualquer suspeita de TypeScript/tsconfig como causa) — reproduzido de forma idêntica tanto via Bash (git-bash) quanto via PowerShell nativo, ou seja, não é um problema de tradução de caminho MSYS/Windows. Chamar o CLI do Playwright diretamente (`node ./node_modules/@playwright/test/cli.js test`), pulando o `npx`, funcionou sem nenhuma outra mudança — indicando que algum wrapper/shim do `npx` **nesta máquina** carrega `@playwright/test` duas vezes. O script `npm run test:e2e` usa a chamada direta para não depender desse comportamento; comentado no `README.md` para quem rodar num ambiente onde `npx playwright test` funciona sem esse problema.

## Requisito 62 — Newsletter

- `NewsletterSubscriber` (Domain, `IAggregateRoot`): `Email` (mesmo value object e `HasConversion` de `Customer`/`Admin`, índice único), `Active`, `CreatedAt`. `Reactivate()`/`Unsubscribe()` são só troca de flag — sem histórico de quando cada mudança aconteceu, por simplicidade (não há uma tela de "histórico de inscrição", só a lista atual de ativos).
- `NewsletterService.SubscribeAsync` é a mesma lógica de idempotência já usada em Wishlist/CartSync: busca por e-mail, se não existe cria, se existe mas está inativo reativa, se já está ativo não faz nada — nunca lança erro para o caso "já inscrito", já que o visitante nem tem como saber se já se inscreveu antes.
- Deliberadamente **não** inclui envio de campanha nenhum — captura e exporta, só isso. Enviar de fato para essa lista é trabalho de uma ferramenta externa (Mailchimp, ou o próprio Resend via seu recurso de Broadcast) que o ateliê ainda não configurou; construir um compositor de campanhas de e-mail dentro do próprio sistema seria um projeto à parte, fora do escopo deste pedido.
- `GET /api/admin/newsletter/export` reaproveita o mesmo padrão de `OrderEndpoints.BuildCsv` (cabeçalho fixo, `CultureInfo.GetCultureInfo("pt-BR")`, `Results.File` com `UTF8Encoding(true)` para o BOM que o Excel espera).
- Formulário de inscrição no rodapé (`PublicLayout`) usa um botão comum com `(click)`, não um `<form (ngSubmit)>` — decisão deliberada depois do bug real encontrado no Requisito 60 (formulário 2FA sem `FormsModule`/`NgForm`); mais simples e sem essa categoria de risco.
- Verificado via curl (inscrição idempotente, listagem, export CSV) e no navegador (inscrição pelo rodapé mostra "Inscrição confirmada!", `/admin/newsletter` lista os inscritos reais).

## Requisito 63 — Cache de imagens e assets estáticos

- `server/ops/nginx-cache-headers.conf`: blocos `location` de referência (não aplicados automaticamente — a configuração do Nginx da VPS não é versionada neste repositório, mesmo padrão dos scripts de backup). Aproveita que dois tipos de arquivo já têm nome imutável por construção: uploads (`Guid.NewGuid()` no nome do arquivo, `LocalFileStorageService`) e o build do Angular (hash de conteúdo no nome, ex. `chunk-B0oRGu4P2.js` — confirmado inspecionando `client/dist/client/browser/` depois de um `npm run build`). Ambos podem levar `Cache-Control: public, max-age=31536000, immutable` sem risco de servir uma versão desatualizada; `index.html` explicitamente `no-cache`, já que ele referencia os nomes com hash da build atual e precisa ser revalidado a cada deploy.
- O bloco de `/api/uploads/` precisa herdar os mesmos `proxy_set_header` (`X-Forwarded-For` etc.) do bloco `/api/` já existente na configuração real da VPS — um `location` mais específico não herda automaticamente de um menos específico no Nginx. Como não tenho acesso a essa configuração (não é deste repositório), o arquivo de referência documenta isso como um passo manual explícito (`sudo nginx -T` para copiar as linhas certas) em vez de assumir o conteúdo.
- CDN (Cloudflare) documentada como melhoria opcional adicional no `README.md`, com os passos exatos, mas **não executada** — trocar os nameservers do domínio é uma ação de alto risco (pode tirar o site do ar durante a propagação, afeta e-mail se houver caixas configuradas no domínio) que só o ateliê deve decidir e disparar, na sua própria conta do registro.br.

## Requisito 64 — Botão de WhatsApp, rastreamento de pedido e selos de segurança

- `WHATSAPP_NUMBER` movido de uma constante privada dentro de `contact.ts` para `core/constants/site.ts` (ao lado de `SITE_NAME`) — passou a ter dois consumidores (`contact.ts` e o novo `<app-whatsapp-button>`), então duplicar o número de telefone em dois arquivos seria um risco de divergência na próxima vez que o ateliê trocar de número.
- `<app-whatsapp-button>` é `position: fixed`, canto inferior direito, `z-index: 1070` — um a menos que o banner de cookies (`1080`), então o banner sempre fica por cima visualmente; o botão lê `CookieConsentService.isDecided` e sobe de `bottom: 24px` para `bottom: 90px` enquanto a escolha ainda não foi feita, pra não ficar embaixo da barra do banner. Renderizado só em `PublicLayout` (nunca no admin).
- Rastreamento de pedido: em vez de expor uma busca por GUID completo (impraticável de digitar) ou uma busca só por e-mail (vazaria todos os pedidos de alguém pro primeiro que soubesse o e-mail), a combinação e-mail + prefixo curto do pedido (os mesmos 8 caracteres já mostrados como "Pedido #xxxxxxxx" em toda a UI) exige as duas informações batendo. `OrderRepository.GetByShortIdAndEmailAsync` filtra por `CustomerEmail` no banco (índice implícito da comparação por valor convertido) e só then compara o prefixo em memória — `Guid.ToString()` dentro de uma expressão LINQ não é traduzível pelo provider SQLite do EF Core, mas o conjunto de pedidos por e-mail de um cliente é pequeno o bastante pra não importar. `GET /api/orders/lookup` tem a mesma política de rate limiting `"auth"` dos outros endpoints "advinhe um segredo" (login, cupom).
- `TrackOrder` (`/rastrear-pedido`) não duplica a UI de detalhe do pedido — depois de encontrar o pedido pelo `LookupAsync`, só navega para `/pedido/:id` (rota já pública, sem guard), reaproveitando a página de confirmação existente.
- Selos de segurança no checkout são só três linhas de texto/ícone abaixo do botão de confirmar — nenhuma lógica nova, puramente para reforçar confiança visualmente no momento mais sensível da compra.

## Requisito 65 — PWA (aplicativo instalável)

- `@angular/service-worker` fixado na versão exata do `@angular/core` já instalado (`22.1.2`) — a resolução `latest` do npm trouxe `22.1.5`, que exige um `@angular/core` na mesma versão exata (peer dependency estrita do pacote), causando um conflito de resolução; o schematic oficial `ng add @angular/pwa` também não serviu (resolveu para a versão `12.2.18`, incompatível com Angular 22, e falhou por uma restrição de `--allow-scripts` do npm neste ambiente) — por isso a integração (config do service worker, manifest, ícones, `angular.json`, `app.config.ts`) foi feita manualmente em vez de via schematic.
- `manifest.webmanifest` (nome, `theme-color`, ícones 192px/512px) em `public/`, referenciado em `index.html`. Os ícones PNG não existiam no projeto (só `favicon.svg`/`favicon.ico`) — gerados renderizando o `favicon.svg` existente num `<canvas>` via `javascript_tool` do Claude-in-Chrome (fetch do SVG, desenhar na resolução alvo, exportar como PNG, baixar) em vez de instalar uma ferramenta de geração de ícone só para isso.
- `ngsw-config.json`: grupo `app` (`installMode: prefetch`) para o shell (HTML/CSS/JS/favicon); grupo `assets` (`installMode: lazy`) para imagens/ícones estáticos; sem `dataGroup` — nenhuma resposta de `/api/**` passa pelo cache do service worker. Um `dataGroup` `freshness` para `/api/products/**` existiu, mas foi removido após causar produtos/fotos desatualizados na loja quando a rede demorava mais que o timeout de 3s (a última resposta cacheada era servida em vez da nova).
- `provideServiceWorker('ngsw-worker.js', { enabled: !isDevMode() })` em `app.config.ts` — nunca ativa em `ng serve`, só no build de produção via `"serviceWorker": "ngsw-config.json"` na configuração `production` do `angular.json`. Confirmado inspecionando `dist/client/browser/` depois de `npm run build`: `ngsw-worker.js`, `ngsw.json` e `manifest.webmanifest` presentes.

## Requisito 66 — Compressão HTTP

- `server/ops/nginx-compression.conf`: mesmo padrão de referência dos outros arquivos em `server/ops/` (não aplicado automaticamente — configuração do Nginx da VPS não é deste repositório). `gzip` habilitado para tipos de texto comuns (CSS/JS/JSON/SVG/XML), nível de compressão 5 (equilíbrio entre CPU e taxa) e tamanho mínimo de 512 bytes (não vale a pena comprimir respostas minúsculas).
- Bloco de Brotli incluído comentado, com os comandos de instalação do módulo (`nginx-module-brotli` via PPA) — Brotli não vem com o Nginx padrão do Ubuntu, então habilitar o bloco sem o módulo instalado quebraria `nginx -t`; documentado como melhoria opcional adicional depois do gzip (que já cobre a maior parte do ganho sem exigir instalar nada).

## Requisito 67 — Produtos relacionados

- `product-detail.ts` busca `ProductService.list(categoria, página 1, 5 itens)` (a mesma listagem paginada da loja) em vez de um endpoint dedicado — pede 5 pra sobrar folga depois de excluir o próprio produto e ainda ficar com até 4. Roda em paralelo às outras chamadas do `ngOnInit` (reviews, elegibilidade, favorito); erro é silencioso (`error: () => {}`) porque sugestões são um extra, não algo que deveria bloquear a página do produto se falhar.

## Requisito 68 — Etiqueta de embalagem

- Reaproveita o padrão CSS `d-print-none`/`d-none d-print-block` do Bootstrap (já importado via `utilities`/`utilities/api`) em vez de criar uma rota de impressão dedicada: a tela normal de detalhe do pedido soma `d-print-none`, e um bloco separado só de etiqueta (`d-none d-print-block`) fica escondido na tela e visível só na impressão — os dois compartilham os mesmos dados já carregados (`order()`, `parsedShippingAddress()`, `parsedItemOptions()`), sem chamada de API extra.
- Primeira versão imprimia a etiqueta ocupando a página inteira (A4 padrão do navegador); o ateliê usa impressora térmica de etiqueta de envio (10x15cm), não impressora comum — corrigido com uma regra `@media print { @page { size: 100mm 150mm; margin: 4mm } }` em `styles.scss`, escopada à classe `.packing-slip` do bloco de etiqueta (fonte reduzida pra caber no formato pequeno). `@page`/tamanho de página impressa não dá pra controlar via classe Bootstrap — precisou de CSS dedicado.

## Requisito 69 — Comprovante de pedido em PDF

- `jsPDF` gera o PDF inteiramente no navegador (`doc.save(...)`, que dispara o download direto) a partir dos mesmos dados já carregados em `order-confirmation.ts` (`Order`) — sem endpoint novo no backend, já que não há nada ali que a API precise calcular ou que não devesse ser exposto (é o mesmo pedido que o cliente já está vendo na tela).
- Layout desenhado manualmente com `doc.text(...)` em coordenadas absolutas (sem um template/HTML-to-PDF) — poucos campos (cliente, endereço, itens, total), não justifica puxar `html2canvas` (dependência do próprio `jsPDF` para converter DOM em imagem) só para isso.
- `jspdf` fixado em `^4.2.1`: a versão inicial `2.5.2` tinha uma vulnerabilidade crítica via `dompurify` (dependência transitiva do recurso de sanitizar/renderizar HTML do jsPDF, não usado aqui); a 4.x remove essa dependência — mesmo cuidado com licenciamento/vulnerabilidade transitiva já aplicado ao ImageSharp (Requisito 39) neste projeto.
