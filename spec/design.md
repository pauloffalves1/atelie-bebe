...
- Rota pública `/pagamento-simulado/:orderId` (`fake-payment.ts`/`.html`) reproduz a decisão visual de uma tela de Checkout Pro — total do pedido, seleção de Pix/boleto/cartão (só estética, não muda o resultado) — com um aviso fixo de "Ambiente de teste" e dois botões, "Simular pagamento aprovado"/"Simular pagamento recusado".
