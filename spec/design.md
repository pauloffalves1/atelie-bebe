...
- O ateliê só quer oferecer Pix e cartão de crédito — como a API do Mercado Pago só permite restringir por exclusão (não existe um "allow-list"), `CreatePreferenceAsync` envia `payment_methods.excluded_payment_types` com todos os outros tipos (`ticket` = boleto, `debit_card`, `prepaid_card`, `digital_wallet`, `digital_currency`, `atm`), deixando só `credit_card` e `bank_transfer` (Pix, no Brasil) disponíveis na página do Checkout Pro.
...
- Sem credenciais reais ainda, `AddInfrastructure` registra `FakePaymentGateway` no lugar de `MercadoPagoGateway` sempre que roda em `Development` **e** `MercadoPago:AccessToken` está vazio — condição dupla, para nunca ativar por acidente em produção (lá, sem token, cai sempre no `MercadoPagoGateway` real, que degrada graciosamente sem nenhum passo de pagamento, nunca numa aprovação fake). `FakePaymentGateway.IsConfigured` é sempre `true`; `CreatePreferenceAsync` devolve uma "preferência" cuja `CheckoutUrl` aponta para a própria SPA (`{App:PublicUrl}/pagamento-simulado/{orderId}`), não para o Mercado Pago.
- Rota pública `/pagamento-simulado/:orderId` (`fake-payment.ts`/`.html`) reproduz a decisão visual de uma tela de Checkout Pro — total do pedido, seleção de Pix/cartão (só estética, não muda o resultado) — com um aviso fixo de "Ambiente de teste" e dois botões, "Simular pagamento aprovado"/"Simular pagamento recusado".
- Esses botões chamam `POST /api/payments/mercadopago/simulate/{orderId}` (`OrderService.simulatePayment`), que só existe quando `MapFakePaymentEndpoints` é registrado em `Program.cs` — condicionado a `app.Environment.IsDevelopment()`, então a rota nem existe no binário publicado em produção. `OrderService.SimulatePaymentAsync` chama `MarkPaymentApproved`/`MarkPaymentRejected` diretamente (com um `ExternalPaymentId` sintético `FAKE-{orderId}`), sem passar pelo `IPaymentGateway` — não há webhook nem chamada de rede nesse caminho, é só para pré-visualizar a UI.
