...
- **`shared/components/`** — cross-feature reusable components. Currently just `pagination/` (`<app-pagination>`, signal `input()`/`output()`, hides itself when `totalPages <= 1`), used by `Shop` and the three admin list screens.
