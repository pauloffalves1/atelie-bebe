...
dotnet test AtelieBebe.slnx                         # runs both test projects (domain + application)
```
...
- **`AtelieBebe.Domain`** — entities (`Product`, `Order`, `OrderItem`, `Customer`, `Admin`, `ContactMessage`), value objects (`Money`, `Email`, `Cpf`), enums (`OrderStatus`, `OrderType`), and domain events. `Entity` (in `Common/`) is the base class holding a private `_domainEvents` list; entities call `AddDomainEvent` internally when their state changes meaningfully (e.g. `Order` created, status changed). `IAggregateRoot` marks entities that repositories are allowed to expose directly — only aggregate roots get a repository.
...
- **`test/AtelieBebe.Domain.Tests`** — xUnit tests covering `Domain` invariants: the `Order` status state machine, `Product` exclusive-access rules, `Money`/`Email`/`Cpf` value objects, and `Customer` registration. Only references `AtelieBebe.Domain`.
- **`test/AtelieBebe.Application.Tests`** — xUnit tests for `Application`-layer logic: `PagedResult<T>`/`Pagination.Normalize` (pagination, added for RF26). References `AtelieBebe.Application`. No `Infrastructure` tests exist yet (repository `Skip`/`Take` queries are only covered by manual/API verification), so mocking a real EF Core query isn't a pattern established in this repo.
...
**Pagination (RF26)**: `GET /api/products`, `/api/admin/products`, `/api/admin/orders`, and `/api/admin/contact-messages` take `page`/`pageSize` query params (defaults set per-endpoint: 12 for the public storefront, 20 for the three admin listings) and return `PagedResult<T>` (`Application/Common/PagedResult.cs`) instead of a bare list. `Pagination.Normalize` (`Application/Common/Pagination.cs`) clamps `page` to ≥1 and `pageSize` to `[1,100]` — call it from any new paginated service method rather than reimplementing the clamp. The corresponding repository methods order first, then `.Skip((page-1)*pageSize).Take(pageSize)`, then `.CountAsync()` for the total — keep that order (count uses the same filtered-but-unpaged query). `ListFeaturedAsync`, `ListCategoriesAsync`, and `ListByCustomerAsync` were deliberately left unpaginated.
...
- **`core/`** — singletons: `services/` (one `HttpClient` wrapper per backend feature, mirroring the Application-layer split — e.g. `product.service.ts` calls both the public `/api/products` and admin `/api/admin/products` endpoints — plus `cep.service.ts`, which is the one service that calls a *third-party* API, ViaCEP, instead of our backend), `models/` (DTO interfaces matching the backend's), `guards/` (`adminGuard`, `customerGuard`, checked in `app.routes.ts`), `interceptors/` (`auth.interceptor.ts` attaches a Bearer token — but only to requests whose URL starts with `environment.apiUrl`; it picks admin vs. customer token based on whether that URL contains `/admin/`. Any request to a third-party host, like `CepService`'s ViaCEP call, is passed through untouched — never attach the interceptor's token check without that `apiUrl` guard, or our JWT leaks to whatever external host a new service calls).
...
- **`shared/components/`** — cross-feature reusable components. Currently just `pagination/` (`<app-pagination>`, signal `input()`/`output()`, hides itself when `totalPages <= 1`), used by `Shop` and the three paginated admin list screens (`/admin/clientes` is a fourth admin list but isn't paginated — it reuses the same non-paginated endpoint as the exclusive-products customer picker).
- **`shared/directives/`** — cross-feature standalone directives. Currently just `phone-mask.directive.ts` (`[appPhoneMask]`), formatting a phone/WhatsApp `<input>` as `(11) 91234-5678` on every keystroke via `NgControl`; applied in `register-page`, `checkout`, and `contact`.
...
`Shop` reflects both filters in the URL query string — `?categoria=` (pre-existing) and `?pagina=` (RF26) — read together in one `queryParamMap` subscription; changing category omits `pagina` from the new navigation, which is what resets to page 1. The three admin list screens keep `page` as local component state instead (no query-string reflection) since back/forward navigation matters less there.
