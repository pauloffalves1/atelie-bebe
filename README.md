...
`MercadoPago:AccessToken` (RF40) segue o mesmo padrão — vazio em `appsettings.json`, configurado localmente via `dotnet user-secrets set "MercadoPago:AccessToken" "<token>" --project src/AtelieBebe.Api` e, em produção, pela variável de ambiente `MercadoPago__AccessToken`. Sem token configurado **em produção**, o checkout de loja funciona normalmente, só sem oferecer pagamento online (`IPaymentGateway.IsConfigured` retorna `false`, e `CreatePreferenceAsync` não é chamado). Em **desenvolvimento** (`dotnet run`), sem token configurado, entra em ação um `FakePaymentGateway` que simula o Checkout Pro pela própria SPA (`/pagamento-simulado/:orderId`) — deixa pré-visualizar o fluxo completo de pagamento antes das credenciais reais existirem, sem chamar nenhuma API externa.
