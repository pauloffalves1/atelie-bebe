...
### Lembrete de carrinho abandonado (RF68)
...
- `Details` mostra o valor antes e depois de cada campo alterado (ex. `Nome: Fralda X → Fralda Y`), não só "o que foi feito" — `AuditDiff` (`Api/Common/`) monta esses fragmentos comparando o estado buscado antes da operação com o DTO devolvido depois dela; campos que não mudaram não aparecem. Isso significa que cada endpoint instrumentado busca o registro atual (`GetByIdAsync`) antes de chamar a operação de escrita, só para ter o "antes" — um custo pequeno (uma consulta a mais) pela rastreabilidade completa.
